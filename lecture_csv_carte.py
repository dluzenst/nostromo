...
def normalize(x):
 #   return unicode((word), "utf-8",errors='ignore')
  #  return uni.normalize('NFKD',word).encode('utf-8','ignore')
 #   x[u'libellevoie']= x[u'libellevoie'].decode(errors='ignore')
 #   return str(word).decode('unicode-escape')
    x[u'libellevoie']= uni.normalize('NFKD',str(x[u'libellevoie']).encode('utf-8','ignore')
    return x
"""
...
def adresse(x):
#    row[u'adresse']  = str(row[u'v1'])+" " +str(row[u'libellevoie'])+" "+str(row[u'Codepos'])+" " +str(row[u'Commune'])+", France"
    x[u'adresse']  = str(x[u'v1'])+" " +str(x[u'libellevoie'])+" "+str(x[u'Codepos'])+" " +str(x[u'Commune'])+", France"
#    x[u'adresse']  = str(x[u'v1'])+" "+str(x[u'Codepos'])+" " +str(x[u'Commune'])+", France"
    #print x[u'adresse']
    return x

"""
# FONCTION QUI RECHERCHE LATITUDE ET LONGITUDE PAR API MAPS
def rech_lat_long(row):
#    adr=str(row[u'adresse'])
    url=url_api_maps+str(row[u'adresse'])+"&key="+API_KEY
    print url
    results_api = requests.get(url)
    johnny = results_api.json()
    print johnny['status']
    if johnny['status']=='OK':
        row['latG'] = johnny['results'][0]['geometry']['location']['lat']
        row['longG'] = johnny['results'][0]['geometry']['location']['lng']
...
columns=[u'numac',u'lum', u'atm', u'com', u'dep', u'v1', u'grav', u'libellevoie', u'lat', u'long']
...
#print df_accn[df_accn[u'dep']>959][u'dep'].value_counts()
...
# Nettoyage
#df_accn[u'v1']=df_accn[u'v1'].apply(normalize)
#df_accn[u'libellevoie']=df_accn[u'libellevoie'].apply(normalize)
### HELP !! Je cherche une regex pour enlever les caractères "bizarres" exemple : GROSSO (JUSQU'AU Nﾰ 58/77) du numacc 15022
...
# TO DO : sélection des lignes des acc qui ont une adresse
#dfshort = df_accn[df_accn[u'libellevoie']!=""]

dfshort = df_accn[15000:15200]
dfvary = df_accn[:][0:40000:200]
#dfshort=dfshort.apply(lambda x: normalize(x),axis=1)
...
def makeColorMap(element_list,element_list2,element_list3):
    ...
    for p, p2, p3 in zip(element_list,element_list2,element_list3):
        ...
        # # # This is for dealing with non-numeric insee - irrelevant for now
        # # idx_selector = df_accn['INSEE'].apply(lambda x : str(x)[0:5])==codeinsee
        # # acSum = df_accn[idx_selector]['grav'].sum()
        # # acCnt = df_accn[idx_selector]['grav'].count()
        # # if(acCnt!=0):
        # #     acBad = acSum/acCnt

        # # print acSum, acCnt, acBad, codeinsee
